...
[[gnu::noinline]] static void apply_cardinals(const vec2c &raw_stick, vec2 *stick)
{
	...
	// Truncate 1.0 cardinals to 0.9875
	if ((raw_x <= -80 || raw_x >= 80) && raw_y == 0)
		*(vec2i*)stick = {std::bit_cast<int>(0.9875f) ^ (std::signbit(raw_stick.x) << 31), 0};
	else if ((raw_y <= -80 || raw_y >= 80) && raw_x == 0)
		*(vec2i*)stick = {0, std::bit_cast<int>(0.9875f) ^ (std::signbit(raw_stick.y) << 31)};
}
